...
namespace Candlestick_Patterns
{
    public class Patterns : IPatterns
    {
        ...
        private readonly OhlcvObject[] _dataArray;
        ...
        public Patterns(List<OhlcvObject> data)
        {
            ...
            _dataArray = data.Select(x => new OhlcvObject()
            {
                Open = x.Open,
                High = x.High,
                Low = x.Low,
            ...
            // Calculate ONCE instead of 16 times
            decimal avgBodySize = data.Select(x => Math.Abs(x.Open - x.Close)).Average();

            _insideBarMaxChange = avgBodySize / 2;
            _minimumCandleSize = avgBodySize * 2;
            _maximumShortCandleSize = avgBodySize / 2;
            _minimumCandleDifference = avgBodySize / 2;
            _maximumDojiShadowSizes = avgBodySize / 2;
            _maximumDojiBodySize = avgBodySize / 2;
            _minimumCandleShadowSize = avgBodySize * 2;
            _maximumShadowSize = avgBodySize * 3;
            _maximumPriceDifference = avgBodySize * 3;
            _maximumCloseDifference = avgBodySize * 3;
            _maximumCandleBodySize = avgBodySize / 2;
            _maximumCandleShadowSize = avgBodySize / 2;
            _maximumDifference = avgBodySize / 2;
            _maximumShadowChange = avgBodySize / 2;
            _maximumCloseHighChange = avgBodySize * 3;
            _maximumCandleSize = avgBodySize / 2;
            _maximumOpenDifference = avgBodySize * 2;
        }

        private List<OhlcvObject> Bearish2Crows()
        {
            var data = new List<OhlcvObject>(_dataArray);// _data.Select(x => new OhlcvObject() { Open = x.Open, High = x.High, Low = x.Low, Close = x.Close, Signal = false, Volume = x.Volume }).ToList();
        ...
        {
            var data = new List<OhlcvObject>(_dataArray);//var data = new List<OhlcvObject>(_dataArray);//var data = _data.Select(x => new OhlcvObject>() { Open = x.Open, High = x.High, Low = x.Low, Close = x.Close, Signal = false, Volume = x.Volume }).ToList();
            for (int i = 5; i < _data.Count; i++)
            {
        ...
        {
            var data = new List<OhlcvObject>(_dataArray);//var data = _data.Select(x => new OhlcvObject>() { Open = x.Open, High = x.High, Low = x.Low, Close = x.Close, Signal = false, Volume = x.Volume }).ToList();
            for (int i = 5; i < _data.Count; i++)
            {
